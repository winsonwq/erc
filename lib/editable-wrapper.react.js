...
class EditableWrapper extends React.Component {
  ...
  renderEditBtn() {
    return <button onClick={ this.props.onSwitchToEditMode }>编辑</button>;
  }

  renderConfirmBtn() {
    return <button onClick={ this.props.onConfirmEditMode }>确定</button>;
  }

  renderCancelBtn() {
    return <button onClick={ this.props.onCancelEditMode }>取消</button>;
  }
