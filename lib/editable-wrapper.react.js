...
class EditableWrapper extends React.Component {
  ...
  renderEditBtn() {
    return <button className="editable-wrapper-button" onClick={ this.props.onSwitchToEditMode }>编辑</button>;
  }

  renderConfirmBtn() {
    return <button className="editable-wrapper-button" onClick={ this.props.onConfirmEditMode }>确定</button>;
  }

  renderCancelBtn() {
    return <button className="editable-wrapper-button" onClick={ this.props.onCancelEditMode }>取消</button>;
  }
  ...
  renderEditContent() {
    ...
    if (editable) {
      if (!editMode) {
        return (
          <span className="editable-wrapper-content non-editmode">
            { this.renderEditBg() }
            { this.renderEditBtn() }
          </span>
        );
  ...
  render() {
    ...
    const clazz = cx('editable-wrapper', className);
