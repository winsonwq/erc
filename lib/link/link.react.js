...
class Link extends M(Basic, EditMode) {
  ...
  renderContent() {
    ...
    const { hrefFieldLabel, altFieldLabel, textFieldLabel, targetFieldLabel, defaultDataText } = this.props;
    ...
    return <a className="erc-link" href={ data.href } alt={ data.alt } target={ data.target ? '_blank' : '_self' }>{ data.text || defaultDataText }</a>;
  }
